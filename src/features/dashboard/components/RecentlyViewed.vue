<template>
  <div class="bg-white rounded-lg shadow-lg p-6">
    ...
    <!-- Empty State -->
    <div v-else-if="recentItems.length === 0" class="text-center py-12">
      <div
      ...
      <p class="text-slate-600 mb-4">
        Start browsing our gemstone collection to see your recently viewed items
        here.
      </p>
      <router-link
        to="/products"
    ...
    >
      ...
      >
        <div
          class="relative overflow-hidden rounded-lg bg-slate-50 aspect-square mb-3"
        >
          <img
            :src="(item.images?.[0] as Image)?.url || item.image"
            :alt="item.name"
          ...
          <div class="absolute top-2 right-2 flex space-x-1">
            <button
              ...
              :class="{
                'text-red-500': item.isWishlisted,
                'text-slate-600': !item.isWishlisted,
              }"
              :title="
        ...
        <div class="space-y-1">
          <h3
            class="font-medium text-slate-800 group-hover:text-emerald-600 transition-colors line-clamp-2"
          >
            {{ item.name }}
          </h3>
    ...
    <!-- Load More -->
    <div v-if="hasMore && recentItems.length > 0" class="text-center mt-8">
      <button
        @click="loadMore"
...
import { GemstoneType } from "@/features/products/store/types";
import type { Image } from "@/features/products/store";
...
const mockRecentItems: RecentItem[] = [
  {
    ...
    cut: "Oval",
    clarity: "VVS1",
    certification: { agency: "GIA", number: "GIA1234567" },
  ...
  },
  {
    ...
    cut: "Round",
    clarity: "VS1",
    certification: { agency: "GRS", number: "GRS789012" },
  ...
  },
  {
    ...
    category: "Tanzanite",
    gemstoneType: GemstoneType.Other,
    price: 6200,
    ...
    images: [
      {
        url: "https://images.unsplash.com/photo-1617038260897-41a1f14a8ca0?w=400",
      },
    ],
    specs: {
      carat: 4.5,
    ...
    cut: "Cushion",
    clarity: "VS2",
    certification: { agency: "GIA", number: "GIA456789" },
...
const addToCart = (item: RecentItem) => {
  if (item.inStock) {
    // Map RecentItem to the older CartItem shape expected by the cart store
    const cartPayload = {
      id: item.id,
      name: item.name,
      price: item.price,
      ...
      image:
        item.image ||
        (item.images && item.images[0] && (item.images[0] as any).url) ||
        "",
      gemType: item.gemstoneType || item.gemType || "",
      carat: item.specifications?.carat || item.carat || 0,
      cut: item.cut || item.specifications?.cut || "",
      color: item.specifications?.color || "",
      clarity: item.clarity || item.specifications?.clarity || "",
    } as any;

    cartStore.addToCart(cartPayload);
  }
};
