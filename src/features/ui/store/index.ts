...
export const useUIStore = defineStore("ui", () => {
  ...
  const isLoading = computed(
    () => loadingStates.value.length > 0 || pageLoading.value
  );
  ...
  const stopLoading = (loadingId: string) => {
    const index = loadingStates.value.findIndex(
      (state) => state.id === loadingId
    );
    if (index > -1) {
  ...
  const setLoading = (loading: boolean) => setPageLoading(loading);
  const toggleCartSidebar = () => toggleSidebar();
