...
export function usePayment() {
  ...
  const isGooglePayAvailable = computed(() => {
    return typeof google !== 'undefined' && google.payments
  })
  ...
  const validatePaymentData = (): boolean => {
    ...
    if (requiresCardData.value) {
      ...
      } else if (!isValidExpiryDate(cardData.expiryMonth, cardData.expiryYear)) {
  ...
  const createPaymentIntent = async (amount: number, currency: string = 'USD'): Promise<PaymentIntent | null> => {
    ...
    try {
      ...
      const intent: PaymentIntent = {
        ...
        metadata: {
          cardBrand: requiresCardData.value ? getCardBrand(cardData.number) : undefined,
