### composables/useImageGallery.ts
```typescript
...
export const useImageGallery = (images: GalleryImage[]) => {
  ...
  // Computed values
  const currentImage = computed(() => images[currentImageIndex.value] || images[0])

  const zoomStyle = computed(() => {
    if (!isZoomed.value) return {}
    
    const translateX = (mousePosition.containerX - mousePosition.x) * zoomLevel.value
    const translateY = (mousePosition.containerY - mousePosition.y) * zoomLevel.value
