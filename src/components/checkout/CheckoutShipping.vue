  ...
  <div class="space-y-8">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-xs text-slate-600 mt-1">
            {{ isGuestCheckout ? 'Checking out as guest' : 'Logged in user checkout' }}
          </p>
        </div>
        <button
    ...
    <div>
      <div class="flex items-center justify-between mb-6">
        ...
        <div v-if="!isGuestCheckout && savedAddresses.length > 0" class="relative">
          <button
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <!-- Address Line 1 -->
        <div class="sm:col-span-2">
          <label for="address1" class="block text-sm font-medium text-slate-700 mb-2">
            Street Address *
          </label>
          <input
            id="address1"
            ...
            required
            class="w-full px-3 py-3 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
            :class="{ 'border-red-300 focus:border-red-500 focus:ring-red-500': errors.address1 }"
        ...
        <!-- Address Line 2 -->
        <div class="sm:col-span-2">
          <label for="address2" class="block text-sm font-medium text-slate-700 mb-2">
        ...
        <!-- ZIP Code -->
        <div>
          <label for="zipCode" class="block text-sm font-medium text-slate-700 mb-2">
            ZIP Code *
          </label>
          <input
            id="zipCode"
            ...
            required
            class="w-full px-3 py-3 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
            :class="{ 'border-red-300 focus:border-red-500 focus:ring-red-500': errors.zipCode }"
    ...
    <div>
      ...
      <div class="space-y-4">
        <div
          ...
          :class="{
            'border-emerald-500 bg-emerald-50': selectedShippingOption === option.id
          }"
        >
          <div class="flex items-center justify-between">
            <div class="flex items-start gap-3">
              ...
              >
              <div>
                <label :for="`shipping-${option.id}`" class="block text-sm font-medium text-slate-800 cursor-pointer">
                  {{ option.name }}
                </label>
          ...
          <div v-if="option.gemstoneFeatures" class="mt-4 pt-4 border-t border-slate-200">
            <div class="flex items-start gap-2">
              <svg class="w-4 h-4 text-amber-600 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
              </svg>
              <div>
                <p class="text-xs font-medium text-amber-800">Luxury Jewelry Features:</p>
    ...
    <div>
      <h2 class="text-xl font-semibold text-slate-800 mb-6">Special Instructions</h2>
      
      <div class="space-y-4">
        <!-- Delivery Instructions -->
        <div>
          <label for="deliveryInstructions" class="block text-sm font-medium text-slate-700 mb-2">
        ...
        <div>
          ...
          <div v-if="isGift" class="mt-4">
            <label for="giftMessage" class="block text-sm font-medium text-slate-700 mb-2">
...
const isFormValid = computed(() => {
  ...
  return requiredFields.every(field => {
    ...
    return value && value.toString().trim() !== '' && !errors[field as keyof typeof errors]
  })
})
...
watch(
  () => ({ ...shippingForm, selectedShippingOption: selectedShippingOption.value }),
  (data) => {
...
const validateField = (fieldName: keyof typeof errors) => {
  ...
  switch (fieldName) {
    case 'firstName':
    case 'lastName':
      errors[fieldName] = !value?.trim() ? `${fieldName === 'firstName' ? 'First' : 'Last'} name is required` : ''
      break
...
const getFeatureBadgeClass = (feature: string): string => {
  ...
  } else if (lowerFeature.includes('priority') || lowerFeature.includes('express')) {
    return 'bg-amber-100 text-amber-800'
  } else if (lowerFeature.includes('premium') || lowerFeature.includes('luxury')) {
