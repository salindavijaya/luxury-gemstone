      ...
      >
        <div class="flex items-start justify-between">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center justify-between mt-2">
                <div class="text-sm text-slate-500">
                  ...
                  <span v-if="option.tracking" class="ml-2"
                    >• Tracking included</span
                  >
                  <span v-if="option.insurance" class="ml-2"
                    >• Insured up to
                    {{ formatCurrency(option.insuranceLimit) }}</span
                  >
                </div>
              </div>
          ...
          <div class="text-right ml-4">
            ...
            <div
              v-if="option.originalPrice && option.originalPrice > option.price"
        ...
        >
          <div class="flex items-center space-x-2 text-sm text-slate-600">
            ...
            <span
              >Estimated delivery:
              {{ formatDate(option.estimatedDelivery) }}</span
            >
          </div>
        </div>
      </div>
      ...
      <div class="flex items-start space-x-2">
        ...
        <div>
          <h4 class="text-sm font-medium text-amber-800">
            Important for Luxury Gemstones
          </h4>
          <p class="text-sm text-amber-700 mt-1">
            All shipments include signature confirmation and tamper-evident
            packaging. For orders over $5,000, we recommend insured shipping
...
const shippingOptions = computed<ShippingOption[]>(() => [
  ...
  },
  {
    ...
    description:
      "White-glove delivery service with armed courier for high-value items",
    price: 149.99,
    deliveryTime: "1-2 business days",
    estimatedDelivery: calculateDeliveryDate(2),
    tracking: true,
...
onMounted(() => {
  if (!selectedOption.value) {
    const recommended = shippingOptions.value.find(
