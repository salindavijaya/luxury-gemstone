  ...
  <div class="space-y-4">
    ...
    <div>
      ...
      <input
        ...
        required
        class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
        :class="{ 'border-red-300 focus:ring-red-500 focus:border-red-500': errors.street }"
    ...
    <!-- Apartment/Unit -->
    <div>
      <label for="apartment" class="block text-sm font-medium text-slate-700 mb-2">
      ...
      <div>
        ...
        >
          ...
          <optgroup label="Canada">
            <option v-for="province in canadaProvinces" :key="province.code" :value="province.code">
              {{ province.name }}
            </option>
          </optgroup>
        </select>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div>
        ...
        <input
          ...
          required
          class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
          :class="{ 'border-red-300 focus:ring-red-500 focus:border-red-500': errors.zipCode }"
    ...
    <div v-if="showVerification && isVerified" class="flex items-center p-3 bg-emerald-50 rounded-lg">
      <CheckCircleIcon class="h-5 w-5 text-emerald-600 mr-2" />
      <span class="text-sm text-emerald-800">Address verified</span>
    </div>

    <div v-if="showVerification && !isVerified && isValidating" class="flex items-center p-3 bg-amber-50 rounded-lg">
...
const isValid = computed(() => {
  ...
  return requiredFields.every(field => localAddress[field as keyof Address]) && Object.keys(errors).length === 0
})
...
const validateAll = (): boolean => {
  ...
  const requiredFields: (keyof ValidationErrors)[] = ['fullName', 'street', 'city', 'state', 'zipCode', 'country', 'phone']
