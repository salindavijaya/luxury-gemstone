### components/ui/PriceCalculator.vue
```vue
<template>
  <div class="space-y-4">
    ...
    <div class="space-y-2">
      ...
      <div class="flex justify-between text-sm">
        <span class="text-slate-600">Tax ({{ Math.round(taxRate * 100) }}%)</span>
      ...
      <div class="border-t border-slate-200 pt-2">
        <div class="flex justify-between">
          ...
          <span class="font-bold text-lg text-slate-900">{{ formatPrice(total) }}</span>
        </div>
      </div>
    </div>
...
const tax = computed(() => (subtotal.value + props.shippingCost) * props.taxRate)
const total = computed(() => subtotal.value + props.shippingCost + tax.value)
