<template>
  <div class="flex flex-col items-center justify-center p-8 md:p-12 text-center">
    ...
    <div class="w-24 h-24 md:w-32 md:h-32 mb-6 relative">
      <div class="w-full h-full bg-gradient-to-br from-slate-100 to-slate-200 rounded-full flex items-center justify-center">
        <ShoppingBagIcon class="w-12 h-12 md:w-16 md:h-16 text-slate-400" />
      </div>
      <!-- Floating sparkles animation -->
      <div class="absolute -top-2 -right-2 w-4 h-4 bg-amber-500 rounded-full opacity-60 animate-pulse"></div>
      <div class="absolute top-2 -left-2 w-2 h-2 bg-emerald-500 rounded-full opacity-60 animate-pulse animation-delay-300"></div>
      <div class="absolute -bottom-1 left-4 w-3 h-3 bg-slate-400 rounded-full opacity-40 animate-pulse animation-delay-600"></div>
    </div>
    ...
    <p class="text-slate-600 mb-6 max-w-md">
      Discover our collection of exquisite gemstones and fine jewelry. 
      Each piece is carefully selected for its exceptional quality and beauty.
    </p>
    ...
    <div v-if="showFeaturedCategories" class="w-full max-w-md mb-6">
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        >
          ...
          <div class="text-xs font-medium text-slate-700 group-hover:text-emerald-700">
            {{ category.name }}
          </div>
        </button>
      </div>
    ...
    <div v-if="showPopularItems && popularItems.length > 0" class="w-full max-w-md mb-6">
      ...
      <div class="space-y-3">
        ...
        >
          ...
          <div class="flex-1 text-left">
            <h4 class="text-sm font-medium text-slate-800 group-hover:text-emerald-700 line-clamp-1">
              {{ item.name }}
            </h4>
          ...
          <ChevronRightIcon class="w-4 h-4 text-slate-400 group-hover:text-emerald-600 transition-colors" />
        </div>
      </div>
    </div>
    ...
    <!-- Trust Indicators -->
    <div class="flex items-center justify-center gap-6 mt-8 pt-6 border-t border-slate-200 w-full max-w-md">
      <div class="flex flex-col items-center">
        <ShieldCheckIcon class="w-6 h-6 text-emerald-600 mb-1" />
        <span class="text-xs text-slate-600 text-center">Certified<br/>Authentic</span>
      </div>
      <div class="flex flex-col items-center">
        <TruckIcon class="w-6 h-6 text-emerald-600 mb-1" />
        <span class="text-xs text-slate-600 text-center">Secure<br/>Shipping</span>
      </div>
      <div class="flex flex-col items-center">
        <ArrowPathIcon class="w-6 h-6 text-emerald-600 mb-1" />
        <span class="text-xs text-slate-600 text-center">30-Day<br/>Returns</span>
      </div>
    </div>

    <!-- Newsletter Signup -->
    <div v-if="showNewsletter" class="w-full max-w-md mt-6 p-4 bg-gradient-to-r from-emerald-50 to-amber-50 rounded-lg border border-emerald-100">
