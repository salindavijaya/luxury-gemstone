<template>
    <div class="w-full max-w-md mx-auto">
      ...
      <form @submit.prevent="handleRegister" class="space-y-6">
        ...
        <div class="relative">
          ...
          <div v-if="errors.firstName" id="firstName-error" class="mt-1 text-sm text-red-600" role="alert">
            {{ errors.firstName }}
          </div>
        </div>
        ...
        <div class="relative">
          ...
          <div v-if="errors.lastName" id="lastName-error" class="mt-1 text-sm text-red-600" role="alert">
            {{ errors.lastName }}
          </div>
        </div>
        ...
        <div class="relative">
          ...
          >
            <svg v-if="showPassword" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L8.464 8.464m1.414 1.414l-7.071 7.071m15.556-7.071l-7.071-7.071m0 0L6.464 3.515m5.657 5.657l7.071 7.071" />
            </svg>
          ...
          <div v-if="form.password && touched.password" id="password-strength" class="mt-2">
          ...
          <div v-if="errors.password" id="password-error" class="mt-1 text-sm text-red-600" role="alert">
            {{ errors.password }}
          </div>
        </div>
        ...
        <div class="relative">
          ...
          <div v-if="errors.confirmPassword" id="confirmPassword-error" class="mt-1 text-sm text-red-600" role="alert">
            {{ errors.confirmPassword }}
          </div>
        </div>
        ...
        <div class="flex items-start space-x-3">
          ...
          <label for="acceptTerms" class="text-sm text-slate-600 leading-5">
            I agree to the 
            <a href="#" class="text-emerald-600 hover:text-emerald-700 underline font-medium">Terms of Service</a> 
            and 
            <a href="#" class="text-emerald-600 hover:text-emerald-700 underline font-medium">Privacy Policy</a>
          </label>
        </div>
        <div v-if="errors.acceptTerms" id="terms-error" class="text-sm text-red-600" role="alert">
          {{ errors.acceptTerms }}
        </div>
        ...
        <div class="flex items-start space-x-3">
          <input
            ...
            :disabled="loading"
          />
          <label for="subscribeNewsletter" class="text-sm text-slate-600 leading-5">
        ...
        <!-- Error Message -->
        <div v-if="submitError" id="submit-status" class="text-sm text-red-600 text-center" role="alert">
          {{ submitError }}
        </div>
      </form>
  ...
  const passwordStrength = computed(() => {
    ...
    return {
      score,
      message: score === 4 ? messages[4] : `${messages[score]} - needs ${feedback.join(', ')}`
    }
  })
  ...
  const validateField = (field: keyof typeof form) => {
    switch (field) {
      ...
      case 'lastName':
        ...
        break
      case 'email':
        errors.email = validateRequired(form.email, 'Email') || validateEmail(form.email)
        break
      case 'password':
        errors.password = validateRequired(form.password, 'Password') || validatePassword(form.password)
        // Re-validate confirm password if it exists
        if (form.confirmPassword) {
          errors.confirmPassword = validatePasswordMatch(form.password, form.confirmPassword)
        }
        break
      case 'confirmPassword':
        errors.confirmPassword = validateRequired(form.confirmPassword, 'Confirm password') || 
