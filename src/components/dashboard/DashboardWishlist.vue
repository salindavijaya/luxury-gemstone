...
<template>
  <div class="space-y-6">
    ...
    <div v-if="collections.length > 0" class="bg-white rounded-lg shadow-lg border border-slate-200">
      ...
      <div class="p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          >
            ...
            <p v-if="collection.description" class="text-sm text-slate-500 truncate">
    ...
    <div v-else-if="filteredItems.length === 0" class="bg-white rounded-lg shadow-lg border border-slate-200 p-12 text-center">
    ...
    >
      <div v-if="shareableCollection" class="space-y-4">
        ...
        <div class="text-xs text-slate-500">
          <p>Anyone with this link can view your collection but cannot make changes.</p>
        </div>
      </div>
    </Modal>
  </div>
...
const filteredItems = computed(() => {
  ...
  return items.value.filter(item => item.collectionId === selectedCollectionId.value);
});

const getCollectionItemCount = (collectionId: string) => {
  return items.value.filter(item => item.collectionId === collectionId).length;
};
