  ...
  >
    ...
    <div class="relative">
      <input
        ...
        class="w-full px-4 py-3 pl-12 pr-10 bg-white border-2 border-slate-200 rounded-lg 
    ...
    >
      <div
        v-if="showSuggestions && (suggestions.length || recentSearches.length || trendingSearches.length)"
        id="search-suggestions"
        class="absolute z-50 w-full mt-2 bg-white rounded-lg shadow-xl border border-slate-200 
               max-h-96 overflow-y-auto"
        role="listbox"
      >
        <!-- Search Suggestions -->
        <div v-if="suggestions.length" class="p-2">
          <div class="text-xs font-medium text-slate-500 uppercase tracking-wide px-3 py-2">
            Suggestions
          </div>
          <button
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="font-medium truncate" v-html="highlightMatch(suggestion.text, query)"></div>
              <div v-if="suggestion.category" class="text-xs text-slate-500 capitalize">
                {{ suggestion.category }}
              </div>
            </div>
            <div v-if="suggestion.count" class="text-xs text-slate-400 flex-shrink-0">
        ...
        <!-- Recent Searches -->
        <div v-if="recentSearches.length && !query" class="p-2 border-t border-slate-100">
        ...
        <!-- Trending Searches -->
        <div v-if="trendingSearches.length && !query" class="p-2 border-t border-slate-100">
          <div class="text-xs font-medium text-slate-500 uppercase tracking-wide px-3 py-2">
            Trending
          </div>
          <button
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-xs text-slate-500">{{ search.trend }}% increase</div>
            </div>
          </button>
        </div>

        <!-- No Results -->
        <div v-if="query && !loading && !suggestions.length" class="p-4 text-center text-slate-500">
...
const mockSuggestions: Suggestion[] = [
  ...
  { text: 'Emerald Gemstones', type: 'product', category: 'gemstones', count: 189 },
  { text: 'Sapphire Necklaces', type: 'product', category: 'jewelry', count: 156 },
...
const getTrendingIndex = (index: number) => suggestions.value.length + Math.min(recentSearches.value.length, 5) + index
...
const fetchSuggestions = async () => {
  ...
  try {
    ...
    // Track performance
    performanceMonitor.trackApiRequest('GET', '/api/search/suggestions', 200, 200)
...
const highlightMatch = (text: string, searchQuery: string): string => {
  ...
  const regex = new RegExp(`(${searchQuery.replace(/[.*+?^${}()|[\]\\]/g, '\\      <div
        v-if="showSuggestions && (suggestions.length || recentSearches.length || trendingSearches.length)"
        id="search')})`, 'gi')
