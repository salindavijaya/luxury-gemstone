### components/gallery/ImageViewer.vue ```vue
<template>
  <div
  ...
  >
    ...
    <!-- Thumbnail Strip -->
    <div
      class="absolute bottom-20 left-1/2 transform -translate-x-1/2 flex space-x-2 bg-black bg-opacity-30 rounded-lg p-2"
    >
      <button
...
const previousImage = () => {
  const newIndex =
    props.currentIndex === 0 ? props.images.length - 1 : props.currentIndex - 1;
  selectImage(newIndex);
};
