<template>
    <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <div
        class="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 z-50"
      >
        <div class="flex items-center space-x-4">
          <div class="flex-1">
            ...
            <p class="text-sm text-slate-600">
              {{ product?.pricePerCarat }} per carat
            </p>
          </div>
          <button
  ...
  <script setup lang="ts">
    ...
    import { useProductDetail } from "@/features/products/composables";
    import { useCart } from "@/features/cart/composables";
        ...
        if (newProduct) {
          ...
          if (metaDescription) {
            metaDescription.setAttribute(
              "content",
              (newProduct.description || '').substring(0, 160)
            );
          }
        }
      },
