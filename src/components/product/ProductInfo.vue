  ...
  <div class="space-y-6">
    ...
    <div>
      <div class="flex items-center space-x-2 mb-2">
        ...
        <span v-if="product.certified" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">
          Certified
        </span>
      </div>
        ...
        <div class="flex items-center space-x-1">
          <div class="flex items-center">
            <StarIcon
              ...
              :key="star"
              class="w-5 h-5"
              :class="star <= product.rating ? 'text-amber-500 fill-current' : 'text-slate-300'"
            />
          </div>
    ...
    <div class="bg-slate-50 rounded-lg p-6">
      ...
      <div class="mt-4 space-y-2">
        <div class="flex items-center space-x-2 text-sm text-slate-600">
          ...
          <span>Starting at {{ formatPrice(product.price / 12) }}/month with financing</span>
        </div>
        <div class="flex items-center space-x-2 text-sm text-slate-600">
    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="text-center p-4 bg-white rounded-lg shadow-sm">
        <div class="text-2xl font-bold text-emerald-600">{{ product.carat }}</div>
        <div class="text-sm text-slate-600">Carats</div>
      </div>
      <div class="text-center p-4 bg-white rounded-lg shadow-sm">
        <div class="text-2xl font-bold text-emerald-600">{{ product.clarity }}</div>
        <div class="text-sm text-slate-600">Clarity</div>
      </div>
      <div class="text-center p-4 bg-white rounded-lg shadow-sm">
        <div class="text-2xl font-bold text-emerald-600">{{ product.color }}</div>
    ...
    <div>
      ...
      <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <dd class="mt-1 text-sm text-slate-900">{{ product.treatment || 'Natural' }}</dd>
        </div>
        <div>
          <dt class="text-sm font-medium text-slate-500">Fluorescence</dt>
          <dd class="mt-1 text-sm text-slate-900">{{ product.fluorescence || 'None' }}</dd>
        </div>
      </dl>
    </div>
    ...
    <div class="border-t border-slate-200 pt-6">
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div class="flex items-center space-x-3">
          ...
          <div>
            <p class="text-sm font-medium text-slate-900">Authentic Guarantee</p>
