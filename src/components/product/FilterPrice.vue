...
watch(
  localPriceRange,
  (val) => {
    console.log("Updated price range filter:", val);
...
const handleDrag = (event) => {
  ...
  if (dragType.value === "min") {
    localPriceRange.value[0] = Math.min(
      newValue,
      localPriceRange.value[1] - stepSize
    );
    minPriceInput.value = localPriceRange.value[0];
  } else {
    localPriceRange.value[1] = Math.max(
      newValue,
      localPriceRange.value[0] + stepSize
    );
  ...
  switch (event.key) {
    ...
    case "ArrowDown":
      ...
      if (type === "min") {
        ...
        newValue = Math.max(
          localPriceRange.value[0] + step,
          localPriceRange.value[1] - step
        );
...
const updatePriceFromInput = () => {
  ...
  minPriceInput.value = Math.max(
    0,
    Math.min(minPriceInput.value || 0, maxPrice)
  );
  maxPriceInput.value = Math.max(
    0,
    Math.min(maxPriceInput.value || maxPrice, maxPrice)
  );
