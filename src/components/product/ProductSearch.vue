<template>
    <div class="relative w-full max-w-md">
      ...
      <div class="relative">
        ...
        <input
          ...
          :class="{
            'ring-2 ring-emerald-500 border-emerald-500': showSuggestions && suggestions.length > 0
          }"
          @input="handleInput"
          @focus="handleFocus"
          @blur="handleBlur"
      ...
      >
        <!-- Popular Searches -->
        <div v-if="showPopularSearches && popularSearches.length > 0" class="p-3 border-b border-slate-100">
        ...
        <div class="py-2">
          ...
          >
            ...
            <!-- Price or Count -->
            <div v-if="suggestion.price || suggestion.count" class="flex-shrink-0 text-right">
              <span
  ...
  const suggestions = computed(() => {
    ...
    const categories = ['Diamond', 'Ruby', 'Sapphire', 'Emerald', 'Tanzanite', 'Opal']
    const categorySuggestions = categories
      ...
      .map(category => ({
        ...
        count: productsStore.products.filter(p => p.gemstoneType === category).length
      }))
  ...
  const scrollToSelectedItem = () => {
    if (selectedIndex.value >= 0) {
      const element = document.getElementById(`suggestion-${selectedIndex.value}`)
      if (element) {
