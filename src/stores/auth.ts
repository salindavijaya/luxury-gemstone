...
class AuthService {
  ...
  async login(credentials: LoginCredentials): Promise<{ user: User; token: string; refreshToken: string }> {
    ...
    if (credentials.rememberMe) {
      ...
      sessionStorage.setItem(AuthService.STORAGE_USER_KEY, JSON.stringify(user))
    }
  ...
  async register(data: RegisterData): Promise<{ user: User; token: string; refreshToken: string }> {
    ...
    sessionStorage.setItem(AuthService.STORAGE_TOKEN_KEY, token)
    sessionStorage.setItem(AuthService.STORAGE_USER_KEY, JSON.stringify(newUser))
  ...
  async updateProfile(userId: string, data: Partial<User>): Promise<User> {
    ...
    const storage = localStorage.getItem(AuthService.STORAGE_USER_KEY) ? localStorage : sessionStorage
  ...
  getStoredAuth(): { user: User | null; token: string | null; refreshToken: string | null } {
    const localStorage_token = localStorage.getItem(AuthService.STORAGE_TOKEN_KEY)
    const localStorage_user = localStorage.getItem(AuthService.STORAGE_USER_KEY)
    const localStorage_refresh = localStorage.getItem(AuthService.STORAGE_REFRESH_KEY)

    const sessionStorage_token = sessionStorage.getItem(AuthService.STORAGE_TOKEN_KEY)
    const sessionStorage_user = sessionStorage.getItem(AuthService.STORAGE_USER_KEY)
